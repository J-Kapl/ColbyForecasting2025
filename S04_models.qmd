...
```{r load_data}
model_input = read_model_input(scientificname = "Xiphias gladius", 
                               approach = "greedy", 
                               mon = "Aug")
...
greedy_model_input = read_model_input(approach = "greedy", mon = month)
cfg = read_configuration(version = sprintf("g_%s", month))
db = brickman_database()
covars = read_brickman(db |> filter(scenario == "PRESENT", interval == "mon"))|>
  select(all_of(cfg$keep_vars))
  
greedy_all_data = prep_model_data(greedy_model_input, 
                           month = month,
                           covars = covars, 
                           form = "table") |>
  select(all_of(c("class", cfg$keep_vars)))

greedy_split_data = initial_split(greedy_all_data, 
                           prop = 3/4,
                           strata = class)

tr_data = training(greedy_split_data)
