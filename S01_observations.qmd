...
material = obs |>
  filter(basisOfRecord == "MaterialSample") |>
  slice(1) |>
  browse_obis()
...
# Counts per month with SST in GoM overlayed
```{r month_ordered}
obs = obs |>
  mutate(month = factor(month, levels = month.abb))

temps = c(8.1, 7.2, 6.5, 7.4, 9.0, 12.7, 17.5, 18.0, 16.6, 14.3, 11.2, 8.5)
temps_df <- data.frame(month = month.abb, temp = temps)

ggplot(data = obs,
       mapping = aes(x = month)) + 
  geom_bar() + 
  labs(title = "Counts per month") +
  geom_hline(data = temps_df, mapping = aes(x = month, y = temp), yintercept = 1500)
```
...
```{r plot_mask}
plot(mask, breaks = "equal", axes = TRUE, reset = FALSE, main = "Swordfish Observations")
plot(st_geometry(obs), pch = ".", add = TRUE)
```

Now I'll try to plot Swordfish geometry by month (order is alphabetical :( )
```{r}
coast = read_coastline()
ggplot(data = obs) + geom_sf(alpha = 0.1, size = 0.7) + geom_sf(data = coast) + facet_wrap(~month) + labs(title = "Swordfish Observations by Month")
```
...
Open the "observations.R" file you'll find in the "functions" directory. We have started it for you. Edit the function so that it appropriately filters your species data set by adding optional arguments (like `minimum_year` has been added).  And then adding the code steps needed to implement that filter.
