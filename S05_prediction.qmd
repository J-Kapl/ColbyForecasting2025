...
# Let's do August in 2055 for both RCP45 and RCP85
```{r load_2055_RCP45}
covars_rcp45_2055 = read_brickman(db |> filter(scenario == "RCP45", year == 2055, interval == "mon")) |>
  select(all_of(cfg$keep_vars)) |>
  slice("month", "Aug") 

forecast_Aug_2055 = predict_stars(wflow, covars_rcp45_2075)
forecast_Aug_2055

coast = read_coastline()
plot(forecast_Aug_2055['.pred_presence'], main = "RCP45 2055 August", 
     axes = TRUE, breaks = seq(0, 1, by = 0.1), reset = FALSE)
plot(coast, col = "orange", lwd = 2, add = TRUE)
```
...
coast = read_coastline()
plot(forecast_rcp45_2075['.pred_presence'], main = "RCP45 2075 August", 
     axes = TRUE, breaks = seq(0, 1, by = 0.1), reset = FALSE)
plot(coast, col = "orange", lwd = 2, add = TRUE)
...
# write individual arrays?
write_prediction(nowcast, file = file.path(path,"g_Aug_RCP45_2020.tif"))
...
# Let's try it for an off-peak month like May
```{r}
cfg = read_configuration(version = "g_May")
db = brickman_database()
covars = read_brickman(db |> filter(scenario == "PRESENT", interval == "mon")) |>
  select(all_of(cfg$keep_vars)) |>
  slice("month", "May")
...
coast = read_coastline()
plot(nowcast['.pred_presence'], main = "Nowcast May", 
     axes = TRUE, breaks = seq(0, 1, by = 0.1), reset = FALSE)
plot(coast, col = "orange", lwd = 2, add = TRUE)
...
forecast_rcp45_2075 = predict_stars(wflow, covars_rcp45_2075)
coast = read_coastline()
plot(forecast_rcp45_2075['.pred_presence'], main = "RCP45 2075 May", 
     axes = TRUE, breaks = seq(0, 1, by = 0.1), reset = FALSE)
plot(coast, col = "orange", lwd = 2, add = TRUE)
...
# write individual arrays?
write_prediction(nowcast, file = file.path(path,"g_May_RCP45_2020.tif"))
write_prediction(forecast_rcp45_2055, file = file.path(path, "g_May_RCP45_2055.tif"))
write_prediction(forecast_rcp45_2075, file = file.path(path, "g_May_RCP45_2075.tif"))
```


# How about December?
```{r}
cfg = read_configuration(version = "g_Dec")
db = brickman_database()
covars = read_brickman(db |> filter(scenario == "PRESENT", interval == "mon")) |>
  select(all_of(cfg$keep_vars)) |>
  slice("month", "Dec")
...
coast = read_coastline()
plot(nowcast['.pred_presence'], main = "Nowcast Dec", 
     axes = TRUE, breaks = seq(0, 1, by = 0.1), reset = FALSE)
plot(coast, col = "orange", lwd = 2, add = TRUE)
...
forecast_rcp45_2075 = predict_stars(wflow, covars_rcp45_2075)
coast = read_coastline()
plot(forecast_rcp45_2075['.pred_presence'], main = "RCP45 2075 Dec", 
     axes = TRUE, breaks = seq(0, 1, by = 0.1), reset = FALSE)
plot(coast, col = "orange", lwd = 2, add = TRUE)
...
# write individual arrays?
write_prediction(nowcast, file = file.path(path,"g_Dec_RCP45_2020.tif"))
write_prediction(forecast_rcp45_2055, file = file.path(path, "g_Dec_RCP45_2055.tif"))
write_prediction(forecast_rcp45_2075, file = file.path(path, "g_Dec_RCP45_2075.tif"))
```

# Just write a loop to make them all for 2055
```{r my_loop}
for (mon in month.abb){
cfg = read_configuration(version = sprintf("g_%s", mon))
db = brickman_database()
covars = read_brickman(db |> filter(scenario == "PRESENT", interval == "mon")) |>
  select(all_of(cfg$keep_vars)) |>
  slice("month", mon)
...
coast = read_coastline()
plot(nowcast['.pred_presence'], main = sprintf("Nowcast %s", mon), 
     axes = TRUE, breaks = seq(0, 1, by = 0.1), reset = FALSE)
plot(coast, col = "orange", lwd = 2, add = TRUE)
...
covars_rcp85_2055 = read_brickman(db |> filter(scenario == "RCP85", year == 2055, interval == "mon")) |>
  select(all_of(cfg$keep_vars)) |>
  slice("month", mon)
forecast_rcp85_2055 = predict_stars(wflow, covars_rcp85_2055)
coast = read_coastline()
plot(forecast_rcp85_2055['.pred_presence'], main = sprintf("RCP85 2055 %s", mon), 
     axes = TRUE, breaks = seq(0, 1, by = 0.1), reset = FALSE)
plot(coast, col = "orange", lwd = 2, add = TRUE)
...
pred_2055 = c(nowcast, forecast_rcp45_2055, forecast_rcp85_2055, along = list(year = c("2020", "RCP4.5 2055", "RCP8.5 2055")))

plot_coast = function(){
  plot(coast, col = "orange", lwd = 2, add = TRUE)
}

plot(pred_2055['.pred_presence'], 
     hook = plot_coast,
     axes = TRUE, breaks = seq(0, 1, by = 0.1), join_zlim  = TRUE, reset = FALSE, 
     main = sprintf("2055 %s", mon))
...
# write individual arrays?
write_prediction(nowcast, file = file.path(path,sprintf("g_%s_RCP45_2020.tif", mon)))
write_prediction(forecast_rcp45_2055, file = file.path(path, sprintf("g_%s_RCP45_2055.tif", mon)))
write_prediction(forecast_rcp85_2055, file = file.path(path, sprintf("g_%s_RCP85_2055.tif", mon)))
}
```
